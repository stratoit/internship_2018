...
objectPoints[:, :2] = np.mgrid[0:cols, 0:rows].T.reshape(-1, 2)
...
for path in glob.glob('camera_cal/*.jpg'):
	...
	if ret:
        	# Refine the corner position(not working)
...
for path in glob.glob('camera_cal/calibration[0-1][0-9].jpg'):
	...
	cv2.imwrite('undistorted/undistorted%d.jpg'%count,undistortedImg)
	count += 1
	#cv2.waitKey(1000)
