# This runs the saved neural network and sends predictions to the arduino during test time for the new neural net.
# Make sure you train the net and create a model.yaml and .h5 folders first and put it in the same folder as this file.
...
def WriteListtoCSV():
	...
	#	defined multiple time to clear buffer
		...
		cropped1 = cv2.resize(frame1,None,fx = 0.25,fy=0.25)
		...
		#data = arr/ 255.0
		ynew = loaded_model.predict_classes(data.reshape(1,60,80,3))
		yprob = loaded_model.predict_proba(data.reshape(1,60,80,3))
  ...
  #infinite loop so that function do not terminate and thread do not end.
	...
	while True: 
		...
		if (y == ord('w')):
			...
			conn.sendall(str(6).encode('utf-8'))
		elif (y == ord('c')):
			conn.sendall(str(8).encode('utf-8'))
		else :
			x = WriteListtoCSV()
			print(x)
			conn.sendall(str(int(x/10)-2).encode('utf-8'))
